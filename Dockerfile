...
# ROOT installation
ADD https://root.cern/download/root_v6.24.02.Linux-ubuntu20-x86_64-gcc9.3.tar.gz /usr/local
ADD /usr/local/root_v6.24.02.Linux-ubuntu20-x86_64-gcc9.3.tar.gz /usr/local
SHELL ["/bin/bash", "-c"]
RUN ls --recursive /usr/local
RUN echo "source /usr/local/root/bin/thisroot.sh" >> ~/.bashrc
RUN source ~/.bashrc && root -v -q
RUN rm -f /usr/local/root_v6.24.02.Linux-ubuntu20-x86_64-gcc9.3.tar.gz
